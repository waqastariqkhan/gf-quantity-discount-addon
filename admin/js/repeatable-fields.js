  ...
  $.fn.repeatable_fields = function (custom_settings) {
      ...
      $("*", new_row).each(function () {
        $.each(this.attributes, function () {
          this.value = this.value.replace(
            self.settings.row_count_placeholder,
            row_count - 1
          );
        });
      });
    ...
    self.initialize = function (parent) {
      $(self.settings.wrapper, parent).each(function () {
        ...
        $(container)
          .children(self.settings.template)
          .hide()
          .find(":input")
          .each(function () {
        ...
        var row_count = $(container)
          .children(self.settings.row)
          .filter(function () {
        ...
        $(wrapper).on("click", self.settings.add, function (event) {
          ...
          var row_template = $(
            $(container)
              .children(self.settings.template)
              .clone()
              .removeClass(self.settings.template.replace(".", ""))[0].outerHTML
          );
        ...
        if (self.settings.is_sortable === true) {
          if (
            ...
            var sortable_options =
              self.settings.sortable_options !== null
                ? self.settings.sortable_options
                : {};
          ...
          $(wrapper)
            .find(self.settings.container)
            .on("click", function (event) {
              if (
                !$(event.target).is(self.settings.move_up) &&
                !$(event.target).is(self.settings.move_down)
              ) {
                return;
              }
              ...
              if (
                $(event.target).siblings(self.settings.move_steps).length === 1
              ) {
                var custom_steps = parseInt(
                  $(event.target).siblings(self.settings.move_steps).val(),
                  10
                );

                if (
                  isNaN(custom_steps) === false &&
                  (custom_steps > 0 || custom_steps === -1)
                ) {
                  steps = custom_steps;
                }
              }
              ...
              if ($(event.target).is(self.settings.move_up) === true) {
                ...
                for (i = 0; steps === -1 ? true : i < steps; i++) {
                  if (previous_row === undefined) {
                    if (
                      current_row.prev().not(self.settings.template).length ===
                      1
                    ) {
                    ...
                    if (
                      previous_row.prev().not(self.settings.template).length ===
                      1
                    ) {
